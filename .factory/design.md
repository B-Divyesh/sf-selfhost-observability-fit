...
- `site/public/telemetry-herbarium.webp`: original raster hero generated for this product with `/opt/fleet/lib/gen-image.sh` using the factory image deployment, then locally converted to a 900 × 900 WebP (quality 76, metadata stripped, 92,314 bytes). Prompt: “Botanical field guide plate on warm uncoated paper: one imaginary telemetry plant growing from a small server rack root system; three distinct branches represented only through forms—rounded trace seedpods, tiny log leaves, circular metric berries—fine engraved ink linework with restrained moss green and muted ochre watercolor washes, subtle paper grain, scientific specimen composition, generous clear negative space, no lettering, no numbers, no logos, no UI, no border, no watermark.” The exact final prompt, deployment, parameters, conversion, and license are preserved in `site/public/telemetry-herbarium.provenance.json`; the shipped WebP is an original project asset licensed MIT with the repository.
